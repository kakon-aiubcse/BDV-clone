                    ...
                    <div className="flex items-center gap-1 text-lg mt-[4px]">
                      <p>
                        ...
                        <strong className="text-sky-500">
                          {categoryNews?.publish_by_data?.name ||
                            "Unknown Author"}
                        </strong>
                    ...
                    >
                      <div className="col-span-1">
                        <p className="text-justify">
                          {stripHtml(
                            categoryNews?.post || "No content available"
                          )}
                        </p>
                      </div>
...
// import { Tab, Tabs, TabList, TabPanel } from "react-tabs";
// import { useEffect, useState } from "react";
// import { fetchnewsDetails } from "../api/connection";
// import Link from "next/link";
...
//   const [relatedNewsData, setRelatedNewsData] = useState(null);
//   const [isModalOpen, setIsModalOpen] = useState(false);
//   const [photoText, setPhotoText] = useState("");
//   const [isMounted, setIsMounted] = useState(false);
...
//       try {
//         setIsLoading(true);
//         const { newsDetails, relatedNewsData } = await fetchnewsDetails();
//         setNewsDetails(newsDetails || {});
//         setRelatedNewsData(relatedNewsData || []);
//       } catch (error) {
...
//     if (
//       typeof window !== "undefined" &&
//       newsDetails?.singleNewsData?.photo_text
//     ) {
//       const parsedText = new DOMParser().parseFromString(
//         newsDetails.singleNewsData.photo_text,
//         "text/html"
...
//                     className="text-[15px] text-center mt-[8px]"
//                     dangerouslySetInnerHTML={{
//                       __html: newsDetails?.photo_text || "No description",
//                     }}
//                   ></p>

//                   <h1 className="text-3xl font-bold mt-3 text-red-600">
//                     {newsDetails?.title || "No Title Available"}
//                   </h1>
...
//                       By{" "}
//                       <strong className="text-sky-500">
//                         {newsDetails?.publish_by_data?.name || "Unknown Author"}
//                       </strong>
//                       <span className="ml-[8px]">
//                         {newsDetails?.published_date
//                           ? formatDate(newsDetails.published_date)
//                           : "Date not available"}
//                       </span>
//                     </p>
//                     <i className="fa-regular fa-bookmark text-xs"></i>
...
//                   <div
//                     className={`grid gap-6 mt-[8px] ${
//                       !newsDetails?.post_2 && !newsDetails?.post_3
//                         ? "grid-cols-1"
//                         : "lg:grid-cols-3 grid-cols-1"
//                     }`}
//                   >
//                     <div className="col-span-1">
//                       <p className="text-justify">
//                         {stripHtml(newsDetails?.post || "No content available")}
//                       </p>
//                     </div>

//                     {newsDetails?.post_2 && (
//                       <div className="col-span-1">
//                         <p className="text-justify">{newsDetails?.post_2}</p>
//                       </div>
//                     )}
...
//                   <div className="h-[120vh] overflow-y-auto scroll_content">
//                     {relatedNewsData?.map((article, index) => (
//                       <Link
//                         href={`/news/newsDetails/${article.id}`}
//                         key={index}
//                         className="flex items-start gap-4 mb-6 hover:bg-gray-50 p-4 rounded-lg transition duration-300"
//                       >
